...
{
	if (CellActors[poz_x * Width + poz_y]->GetCellValue() != 9)
	{
		CellActors[poz_x * Width + poz_y]->SetCellValue(9); //ustawiamy mine
		CellActors[poz_x * Width + poz_y]->SetCellColorMine();
		...
		for (int k = -1; k < 2; k++)
			for (int l = -1; l < 2; l++)
			{
				...
				if ((poz_x + l) > Width-1 || (poz_y + k) > Height-1) continue; //wyjdz bo krawedz


				if (CellActors[(poz_x + l) * Width + poz_y + k]->GetCellValue() == 9) continue; //wyjdz bo mina
				int32 TempValue = CellActors[(poz_x + l) * Width + poz_y + k]->GetCellValue();
				CellActors[(poz_x + l) * Width + poz_y + k]->SetCellValue(TempValue + 1);
				CellActors[(poz_x + l) * Width + poz_y + k]->SetCellColor();
...
{
	UE_LOG(LogTemp, Warning, TEXT("PosX=%d, PosY=%d"), x, y);
	if (x < 0 || x>Width - 1) return; // poza tablic¹ wyjœcie
	if (y < 0 || y>Height - 1) return; // poza tablic¹ wyjœcie
	if (CellActors[x * Width + y]->GetCellVisible() == true) return;  // ju¿ odkryte wyjœcie
	
	if (CellActors[x * Width + y]->GetCellValue() != 9 && CellActors[x * Width + y]->GetCellVisible() == false)
		CellActors[x * Width + y]->SetCellVisible(true) ;   // odkryj!
	
	if (CellActors[x * Width + y]->GetCellValue() != 0) return; // wartoœæ > 0 wyjœcie
